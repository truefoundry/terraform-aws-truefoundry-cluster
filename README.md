...
| <a name="input_cluster_encryption_config"></a> [cluster\_encryption\_config](#input\_cluster\_encryption\_config) | Configuration for encryption of the cluster. To disable this, set this to {}. For passing a custom key, set the value to { "resources": [ "secrets" ], "provider\_key\_arn": "" } | `any` | <pre>{<br/>  "resources": [<br/>    "secrets"<br/>  ]<br/>}</pre> | no |
...
| <a name="input_cluster_kms_key_override_policy_documents"></a> [cluster\_kms\_key\_override\_policy\_documents](#input\_cluster\_kms\_key\_override\_policy\_documents) | List of IAM policy documents that are merged together into the cluster KMS key policy. In merging, statements with non-blank `sid`s will override statements with the same `sid` | `list(string)` | `[]` | no |
...
| <a name="input_initial_node_pool_ami_release_version"></a> [initial\_node\_pool\_ami\_release\_version](#input\_initial\_node\_pool\_ami\_release\_version) | Exact AMI release version for the initial node pool (e.g. 1.34.10-20260827). Only applied when initial\_node\_pool\_use\_latest\_ami\_release\_version is false; ignored while that is true (the default). | `string` | `""` | no |
...
| <a name="input_initial_node_pool_enable_monitoring"></a> [initial\_node\_pool\_enable\_monitoring](#input\_initial\_node\_pool\_enable\_monitoring) | Enable monitoring for the initial node pool | `bool` | `false` | no |
...
| <a name="input_initial_node_pool_use_latest_ami_release_version"></a> [initial\_node\_pool\_use\_latest\_ami\_release\_version](#input\_initial\_node\_pool\_use\_latest\_ami\_release\_version) | Whether the initial node pool tracks the latest EKS-optimized AMI for its ami\_type. Default true keeps it on latest, which causes in-place release\_version drift as new AMIs are published. Set false to stop the drift: freezes on the current AMI | `bool` | `true` | no |
